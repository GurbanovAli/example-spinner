...
WinPanel.prototype.init = function() {
   this.visible = true;
   if (!this.state && this.count < getScore && this.count >= 0) {
      this.count += 0.5;
      this.countingText.text = Math.floor(this.count);
   } else if (!this.state && this.count < getScore + 40 && this.count >= 0) {
      this.count += 1;
      ...
      this.count -= 0.5;
      this.countingText.text = Math.ceil(this.count);
